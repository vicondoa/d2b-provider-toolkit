...
| `d2b_provider_sdk::contracts::provider` | `d2b-contracts/v2-provider` | Provider DTOs and axis traits |
| `d2b_provider_sdk::contracts::identity` | `d2b-contracts/v2-provider` dependency | Typed provider, realm, workload, and role identities |
| `d2b_provider_sdk::contracts::component_session` | `d2b-contracts/v2-provider` dependency | Placement roles and session contract values |
...
Canonical `d2b-provider-toolkit` internally enables
`d2b-contracts/v2-services` for generated provider service servers. The facade
also selects `d2b-contracts/v2-provider` explicitly so its direct serialized
surface is reviewable in `Cargo.toml`.
